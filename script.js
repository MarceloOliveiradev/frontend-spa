...
async function consultarFrete() {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:5000/frete/${cep}`);
    ...
    const freteDiv = document.getElementById("freteResult");
    freteDiv.innerHTML = `
      <h3>Frete Calculado</h3>
      <p><strong>Endereço:</strong> ${data.logradouro}, ${data.bairro} - ${
      data.localidade
    }/${data.uf}</p>
      <p><strong>CEP:</strong> ${data.cep}</p>
      <p><strong>Valor do Frete:</strong> R$ ${data.frete.toFixed(2)}</p>
    `;
  } catch (error) {
